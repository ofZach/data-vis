...
void testApp::searchGoogleImages() {
    ...
    if(res.status > 0) {
        ...
        if(found != 0) {
            ...
            RegularExpression regExForLarge("imgurl.*?&");
            RegularExpression::Match largeMatch;
            int count = 0;
            while(regExForLarge.match(contentsCopy, largeMatch) != 0) {
                
                string foundStr = contentsCopy.substr(largeMatch.offset, largeMatch.length);
                foundStr.erase(foundStr.end()-1);
                foundStr.erase(foundStr.begin(), foundStr.begin()+7);
                cout << foundStr << endl;
                

                // todo parse file extension 
                
                
                
                string command = "curl -L -o " + ofToDataPath("image-" + ofToString(count)) + " " +  foundStr + " &";
